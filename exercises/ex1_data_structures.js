...
names.push(newName);
URLs[URLs.length] = newURL; //uses index number (arrays start at 0)
years = years.concat([newYear]); //concatenating to an array
...
for (var i = 0; i < names.length; i++) {
  ...
  museums[currentName] = {};
  museums[currentName]["URL"] = currentURL; //["URL"] is like that because it may have special characters (do not use ".")
...
var museums2 = {};
names.forEach(function(n,i) { //first is iterator n , second is count i
  museums2[n] = {};
...
addAMuseum(museums2,"Smithsonian Archives","example.com",1891);
